    ...
    bool doShapesIntersect(const std::vector<sf::Vector2f>& shape1, const std::vector<sf::Vector2f>& shape2) {
        ...
        for(size_t t = 0; t < 2; ++t){
            ...
            for(size_t i = 0; i < s1->size(); i++) {
                sf::Vector2f perpendicular = getPerpendicular(sf::Vector2f((*s1)[(i + 1) % s1->size()].x - (*s1)[i].x, (*s1)[(i + 1) % s1->size()].y - (*s1)[i].y));
                float min1 = FLOAT_INFINITY;
                float max1 = -FLOAT_INFINITY;
                for(size_t j = 0; j < s1->size(); j++) {
                    float dp = dotProduct(perpendicular, (*s1)[j]);
                ...
                for(size_t j = 0; j < s2->size(); j++) {
                    float dp = dotProduct(perpendicular, (*s2)[j]);
